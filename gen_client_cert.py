...
def gen_client_cert(config):
    ...
    for client in clients:
        ...
        logging.info('Start generation cloud certificate: {}'.format(new_alias))
        ...
        with open(store_folder+new_alias+".p12", "wb") as f:
            f.write(p12)
        logging.info('End of generation cloud certificate: {}'.format(new_alias))
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
                        description='Generate client certificate(s).'
                        )
    parser.add_argument("-p", "--config-path", type=str, default="config/client_certifi_req.json",
